...
Joystick *RemoteController::getJoystick()
{
        ...
        if (cap->read(frame)) {
            if (!frame.empty()) {
                if (m_watermarkOn) {
                    ...
                    static double watermarkScale = 0.35;
...
int RemoteController::getSpeedMode() {
    ...
    if (Joystick::instance->getButtonValue(JoystickAxes::SpeedSlow)) speed = static_cast<int>(SpeedModes::Low);
    if (Joystick::instance->getButtonValue(JoystickAxes::SpeedFast)) speed = static_cast<int>(SpeedModes::Max);
