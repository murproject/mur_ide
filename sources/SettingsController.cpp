...
//qml::RegisterType<SettingsController> SettingsController::Register;
...
SettingsController::SettingsController()
{
    ...
    if (os == "winnt") {
        ...
        if (!isValidPath(m_simulatorPath)) {
            m_simulatorPath = path.absolutePath() + "/simulator/murSimulator.exe";
        }
    }
    ...
    if (os == "linux") {
        ...
        if (!isValidPath(m_simulatorPath)) {
            m_simulatorPath = "simulator/simulator"; // try local build first

            if (!isValidPath(m_simulatorPath)) {
                m_simulatorPath = "mur-ide.simulator"; // snapped simulator
            }
        }
    }
    ...
    qInfo() << "paths:"        << endl
            << "  resources: " << m_resourcePath  << endl
            << "  python:    " << m_pythonPath    << endl
            << "  simulator: " << m_simulatorPath << endl;
}
