...
Rectangle {
    ...
    Item {
        ...
        Keys.onPressed: {
            if (event.modifiers & Qt.CtrlModifier) {
                Controllers.gamepad.setForceAxisValue(GamepadAxes.SpeedFast, 100);
            }

            if (event.modifiers & Qt.ShiftModifier) {
                Controllers.gamepad.setForceAxisValue(GamepadAxes.SpeedSlow, 100);
            }
            ...
            if (action !== undefined) {
                ...
                Controllers.gamepad.addForceAxisValue(action[0], power);
            }
        }
        ...
        Keys.onReleased: {
            ...
            if (!event.modifiers && !event.isAutoRepeat) {
                Controllers.gamepad.setForceAxisValue(GamepadAxes.SpeedFast, 0);
                Controllers.gamepad.setForceAxisValue(GamepadAxes.SpeedSlow, 0);
            }

            if (action !== undefined && !event.isAutoRepeat) {
                Controllers.gamepad.setForceAxisValue(action[0], 0);
            }
        }
    }
    ...
    GamepadWidget {
        ...
        anchors.verticalCenter: parent.verticalCenter;
        anchors.horizontalCenter: parent.horizontalCenter;
        opacity: (keyboardControl.activeFocus || Controllers.gamepad.gamepadConnected) && !(!keyboardControl.activeFocus && ma.containsMouse) ? 1.0 : 0.5;
        widgetHeight: 60;
